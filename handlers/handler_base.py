...
- 注册路由：/tts/speakers, /tts/clones（handler 特有路由）
- 启动 worker 守护线程
...
def _worker_loop(config):
    """从 task_manager 取任务，调用 model.generate_voice_clone() 生成音频"""
    import soundfile as sf
...
def start_worker(config, task_manager):
    """启动 worker 守护线程"""
    global _task_manager, _output_base, _clone_dir_base
