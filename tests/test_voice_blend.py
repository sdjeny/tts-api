"""
Kokoro Issue #1 测试：音色加权混合 + Speed 语速参数
...
测试覆盖：
  ...
  Test 4: 加权混合（2:1）
  Test 5: 多音色加权（3个不同权重）
  Test 6: speed 参数（0.5 / 1.0 / 1.5 / 2.0）
  Test 7: 异常情况
  Test 8: 边界情况
"""
...
def submit_and_wait(text, speaker, language="en", speed=None, **kwargs):
...
print()
print("=" * 60)
print("Test 2: 单音色生成（基线）")
print("=" * 60)

single_sizes = {}

for spk in ["af_heart", "am_adam", "af_bella"]:
    print(f"  [{spk}] ...", end=" ", flush=True)
    data, err = submit_and_wait(EN_TEXT, spk, language="en")
    if err:
        record(False, f"{spk}: {err}")
    else:
        path = save_wav(data, f"single_{spk}.wav")
...
print()
print("=" * 60)
print("Test 3: 等权混合（+ 号语法，2个音色）")
print("=" * 60)

print("  [af_heart+am_adam] 等权混合 ...", end=" ", flush=True)
data, err = submit_and_wait(EN_TEXT, "af_heart+am_adam", language="en")
if err:
    record(False, f"等权混合: {err}")
else:
    path = save_wav(data, "blend_eq_af_heart_am_adam.wav")
    size = len(data)
    # 验证：混合后的音频大小应与单音色不同（字节级别可能相同，仅做合理性检查）
    record(size > 0, f"等权混合 af_heart+am_adam: {size} bytes")

print("  [af_bella+af_heart] 等权混合（双女声） ...", end=" ", flush=True)
data, err = submit_and_wait(EN_TEXT, "af_bella+af_heart", language="en")
if err:
    record(False, f"等权混合: {err}")
else:
    path = save_wav(data, "blend_eq_af_bella_af_heart.wav")
    record(len(data) > 0, f"等权混合 af_bella+af_heart: {len(data)} bytes")
...
print()
print("=" * 60)
print("Test 4: 加权混合（2:1）")
print("=" * 60)

print("  [af_heart(2)+am_adam(1)] 加权 2:1 ...", end=" ", flush=True)
data, err = submit_and_wait(EN_TEXT, "af_heart(2)+am_adam(1)", language="en")
if err:
    record(False, f"加权混合: {err}")
else:
    path = save_wav(data, "blend_weighted_2_1.wav")
    record(len(data) > 0, f"加权混合 2:1: {len(data)} bytes")

# 对比：等权 vs 加权（应产生不同音频）
print("  [对比] 等权 vs 加权 ...", end=" ", flush=True)
data_eq, err_eq = submit_and_wait(EN_TEXT, "af_heart+am_adam", language="en")
data_wt, err_wt = submit_and_wait(EN_TEXT, "af_heart(2)+am_adam(1)", language="en")
if err_eq or err_wt:
    record(False, f"对比失败: eq_err={err_eq}, wt_err={err_wt}")
else:
    # 文件大小应不同（因为权重不同导致音频不同）
...
blend_3_spec = "af_heart(2)+am_adam(1)+af_bella(3)"
print(f"  [{blend_3_spec}] 三音色加权 ...", end=" ", flush=True)
data, err = submit_and_wait(EN_TEXT, blend_3_spec, language="en")
if err:
    record(False, f"三音色加权: {err}")
else:
    path = save_wav(data, "blend_3way_weighted.wav")
    record(len(data) > 0, f"三音色加权: {len(data)} bytes")

# 四音色
blend_4_spec = "af_heart(1)+am_adam(1)+af_bella(1)+am_onyx(1)"
print(f"  [{blend_4_spec}] 四音色等权 ...", end=" ", flush=True)
data, err = submit_and_wait(EN_TEXT, blend_4_spec, language="en")
if err:
    record(False, f"四音色等权: {err}")
else:
    path = save_wav(data, "blend_4way_eq.wav")
...
for spd, desc in speed_tests:
    ...
    data, err = submit_and_wait(EN_TEXT, "af_heart", language="en", speed=spd)
    if err:
...
data, err = submit_and_wait(EN_TEXT, "af_heart", language="en")
if err:
...
# 7a: 空 speaker（应使用默认 af_heart）
print("  [7a] 空 speaker ...", end=" ", flush=True)
data, err = submit_and_wait(EN_TEXT, "", language="en")
if err:
...
data, err = submit_and_wait(EN_TEXT, "nonexistent_voice_xyz", language="en")
if err:
...
data, err = submit_and_wait(EN_TEXT, "af_heart(1)+nonexistent_xyz(1)", language="en")
if err:
    record(True, f"正确报错: {err}")
else:
...
r = client.submit(text="", speaker="af_heart", language="en")
if r.error:
...
r = client.submit(text=EN_TEXT, speaker="af_heart", language="en", speed=0.1)
if r.error:
    record(True, f"正确拒绝 speed=0.1: {r.error}")
else:
    # 服务端可能不校验，在 worker 中出错
    sr = client.wait(r.task_id)
    if sr.status == "failed":
...
r = client.submit(text=EN_TEXT, speaker="af_heart", language="en", speed=10.0)
if r.error:
...
r = client.submit(text=EN_TEXT, speaker="af_heart", language="en", speed="fast")
if r.error:
...
data, err = submit_and_wait(EN_TEXT, "af_heart(0)+am_adam(1)", language="en")
if err:
...
# 8a: 极端权重比（1000:1）
print("  [8a] 极端权重比 1000:1 ...", end=" ", flush=True)
data, err = submit_and_wait(EN_TEXT, "af_heart(1000)+am_adam(1)", language="en")
if err:
    record(False, f"极端权重失败: {err}")
else:
    path = save_wav(data, "blend_extreme_1000_1.wav")
    record(True, f"极端权重 1000:1: {len(data)} bytes")

# 8b: 极端权重比（1:1000）
print("  [8b] 极端权重比 1:1000 ...", end=" ", flush=True)
data, err = submit_and_wait(EN_TEXT, "af_heart(1)+am_adam(1000)", language="en")
if err:
    record(False, f"极端权重失败: {err}")
else:
    path = save_wav(data, "blend_extreme_1_1000.wav")
    record(True, f"极端权重 1:1000: {len(data)} bytes")

# 8c: speed 边界 - 最小值 0.25
print("  [8c] speed 边界 - 最小值 0.25 ...", end=" ", flush=True)
data, err = submit_and_wait(EN_TEXT, "af_heart", language="en", speed=0.25)
if err:
...
# 8d: speed 边界 - 最大值 4.0
print("  [8d] speed 边界 - 最大值 4.0 ...", end=" ", flush=True)
data, err = submit_and_wait(EN_TEXT, "af_heart", language="en", speed=4.0)
if err:
...
# 8e: speed 正好等于 0.25（下边界）
print("  [8e] speed=0.2499（低于下限） ...", end=" ", flush=True)
r = client.submit(text=EN_TEXT, speaker="af_heart", language="en", speed=0.2499)
if r.error:
...
# 8f: 加权混合 + speed 组合
print("  [8f] 加权混合 + speed 组合 ...", end=" ", flush=True)
data, err = submit_and_wait(EN_TEXT, "af_heart(2)+am_adam(1)", language="en", speed=1.5)
if err:
    record(False, f"组合测试失败: {err}")
else:
    path = save_wav(data, "blend_weighted_speed_1.5.wav")
    record(True, f"加权混合 + speed=1.5: {len(data)} bytes")

# 8g: 中文音色混合
print("  [8g] 中文音色混合 ...", end=" ", flush=True)
data, err = submit_and_wait(ZH_TEXT, "zf_xiaobei+zf_xiaoni", language="zh")
if err:
    record(False, f"中文音色混合失败: {err}")
else:
    path = save_wav(data, "blend_zh_xiaobei_xiaoni.wav")
    record(True, f"中文音色混合: {len(data)} bytes")

# 8h: 中文加权混合
print("  [8h] 中文加权混合 ...", end=" ", flush=True)
data, err = submit_and_wait(ZH_TEXT, "zf_xiaobei(3)+zm_yunjian(1)", language="zh")
if err:
    record(False, f"中文加权混合失败: {err}")
else:
    path = save_wav(data, "blend_zh_weighted.wav")
    record(True, f"中文加权混合: {len(data)} bytes")
